...
export const mapProvinceToCity = new Map(
  districtData.map((v) => [v.name, v.city.map((vv) => vv.name)]),
);
export const mapCityToArea = new Map(
  districtData.flatMap((v) => v.city).map((v) => [v.name, v.area]),
);
export const provinces = districtData.map((v) => v.name);
